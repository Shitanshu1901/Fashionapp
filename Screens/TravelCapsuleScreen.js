...
const buildCapsule = (wardrobe, userGender, occasion, days, wornIds) => {
  ...
  const pool = wardrobe.filter(item => {
    if (item.archived || wornIds.has(item.id)) return false;
  ...
  const getSlot = (slotName) =>
    pool.filter(i => {
      ...
      return itemSlot === slotName || (slots[slotName] || []).includes(subCat);
    });

  const tops    = getSlot('top');
  const bottoms = getSlot('bottom');
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
      {items.map((item, i) => (
        <View key={i} style={styles.capsuleItem}>
          ...
          <Text style={styles.capsuleCat} numberOfLines={1}>
...
export default function TravelCapsuleScreen({ userGender }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setWardrobe(wb || []); setWornIds(worn);
    };
    load();
  }, []);
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.inner} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.daysRow}>
          <TouchableOpacity style={styles.dayBtn} onPress={() => { if (days > 1) { setDays(d => d - 1); setCapsule(null); } }}>
          ...
          <TouchableOpacity style={styles.dayBtn} onPress={() => { if (days < 14) { setDays(d => d + 1); setCapsule(null); } }}>
            <Text style={styles.dayBtnText}>+</Text>
          </TouchableOpacity>
        </View>
        ...
        {capsule && (
          <View>
            ...
            <Text style={styles.resultsCaption}>
              {capsule.totalPieces} pieces creating {capsule.combinations} outfit combinations — perfect for your {days}-day trip.
            </Text>
...
const styles = StyleSheet.create({
  ...
  capsuleCat:   { fontSize: 10, fontWeight: '700', color: '#1A1A1A' },
