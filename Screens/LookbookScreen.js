...
import { OUTFIT_SLOTS, areColorsHarmonious } from '../utils/StyleRules';
...
export default function LookbookScreen({ userGender }) {
  ...
  const findMatches = () => {
    if (!pickedColors.length) {
      Alert.alert('Pick Colors', 'Select 1–3 colors you see in the reference photo.');
      return;
    }
    // Find wardrobe items matching any of the picked colors
    const matches = wardrobe.filter(item =>
      item.color && pickedColors.some(c =>
        item.color === c ||
        (c === 'Black' && item.color === 'Black') ||
        areColorsHarmonious(item.color, c)
      )
    );
  ...
  const renderHistoryItem = ({ item }) => {
    const top   = item.outfit.top    || item.outfit.shirt;
    const bot   = item.outfit.bottom || item.outfit.pants;
    const shoe  = item.outfit.shoes;
    return (
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          ...
          <TouchableOpacity onPress={() => handleFavorite(item.id)} style={styles.heartBtn}>
            <Text style={styles.heartIcon}>{item.favorited ? '❤️' : '🤍'}</Text>
          </TouchableOpacity>
        </View>
        <View style={styles.outfitRow}>
          {[top, bot, shoe].map((piece, idx) => (
            <View key={idx} style={styles.pieceWrapper}>
              ...
              {!!piece?.category && <Text style={styles.pieceCategory}>{piece.category}</Text>}
            </View>
          ))}
        </View>
      </View>
          ...
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtext}>
              {showFavOnly
                ...
                : 'Generate an outfit and tap ❤️ Save to start building your history.'}
            </Text>
          </View>
        }
      />
          ...
          <FlatList
                  ...
                  <>
                    ...
                    <View style={styles.colorGrid}>
                      {ITEM_COLORS.map(color => (
                        ...
                        >
                          <Text style={[styles.colorChipText, pickedColors.includes(color) && styles.colorChipTextActive]}>
                            {color}
                          </Text>
                        </TouchableOpacity>
                      ))}
            ...
            renderItem={({ item }) => (
              <View style={styles.matchCard}>
                ...
                <Text style={styles.matchCat}>{item.category}</Text>
...
const styles = StyleSheet.create({
  ...
  pieceCategory:{ fontSize: 10, color: '#C0B8B0', marginTop: 1 },
  ...
  refPhotoBox:     { height: 220, backgroundColor: '#EFEAE2', borderRadius: 20, justifyContent: 'center', alignItems: 'center', marginBottom: 20, overflow: 'hidden' },
  ...
  matchCat:        { fontSize: 10, fontWeight: '700', color: '#1A1A1A' },
